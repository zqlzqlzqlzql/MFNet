...
parser.add_argument('--dataset', choices=['Houston2013', 'Trento', 'Augsburg'], default='Augsburg', help='dataset to use')
...
parser.add_argument('--num_classes', default=7, help='dataset to use')
...
if args.dataset == 'Houston2013':
    ...
    data_hsi = loadmat('F:\pythonProject\datasets\Multisource\data\HS-SAR-DSM Augsburg\data_HS_LR.mat')
    data_HSI = data_hsi['data_HS_LR']
    data_lidar = loadmat('F:\pythonProject\datasets\Multisource\data\HS-SAR-DSM Augsburg\data_DSM.mat')
    data_liDAR = data_lidar['data_DSM']
    groundtruth = loadmat('F:\pythonProject\datasets\Multisource\data\HS-SAR-DSM Augsburg\gt.mat')
    groundtruth = groundtruth['combined_gt']
    color_mat = loadmat('F:\pythonProject\datasets\Multisource\data\HS-SAR-DSM Augsburg\AugsburgColormap.mat')
else:
...
data_HSI = np.transpose(data_HSI1, (2,0,1)) #shape:(144,349,1905)
...
data = data_HSI.reshape(np.prod(data_HSI.shape[:1]),np.prod(data_HSI.shape[1:])) #shape: (144,664845)
data = MinMaxScaler(feature_range=(-1, 1)).fit_transform(data)
new_gt = groundtruth
gt = new_gt.reshape(np.prod(new_gt.shape[:2]),) #(664845,)

whole_data = data.reshape(data_HSI.shape[0], data_HSI.shape[1],data_HSI.shape[2]) #(144,349,1905)
